...
public final class Skew implements ISuffixArrayBuilder {
    ...
    /**
     * Stably sort indexes from src[0..n-1] to dst[0..n-1] with values in 0..K from
     * v. A constant offset of <code>vi</code> is added to indexes from src.
     */
    private final static void radixPass(int[] src, int[] dst, int[] v, int vi, final int n, final int K, int start,
            int[] cnt) {
    ...
    static final int[] suffixArray(int[] s, int[] SA, int n, final int K, int start, int[] cnt) {
        ...
        /*
         * generate positions of mod 1 and mod 2 suffixes the "+(n0-n1)" adds a dummy
         * mod 1 suffix if n%3 == 1
         */
        ...
        for (int p = 0, t = n0 - n1, k = 0; k < n; k++) {
            ...
            if (SA12[t] < n0 ? leq(s[start + i], s12[SA12[t] + n0], s[start + j], s12[j / 3])
                    : leq(s[start + i], s[start + i + 1], s12[SA12[t] - n0 + 1], s[start + j], s[start + j + 1],
                            s12[j / 3 + n0])) {
    ...
    /**
     * Throw {@link AssertionError} if a condition is <code>false</code>. This
     * should be called when the assertion must be always verified (as in the case
     * of verifying the algorithm's preconditions). For other, internal assertions,
     * one should use <code>assert</code> keyword so that such assertions can be
     * disabled at run-time (for performance reasons).
     */
     ...
     * <li><code>input.length</code> &gt;= <code>start + length + 3</code> (to
     * simplify border cases)</li>
     * <li>length >= 2</li>
     * </ul>
     * <p>
    ...
    public int[] buildSuffixArray(int[] input, int start, int length) {
        ...
        assert allPositive(input, start, length);
