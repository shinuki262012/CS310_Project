...
public interface ISuffixArrayBuilder {
     ...
     * Also, some algorithms may require non-negative symbols in the input. For such
     * constrained algorithms, use various decorators and adapters available in this
     * package.
     * 
     ...
     * @return An array of indices such that the suffix of <code>input</code> at
     *         index <code>result[i]</code> is lexicographically larger or equal to
     *         any other suffix that precede it. Note that the output array may be
     *         larger than <code>input.length</code>, in which case only the first
     *         <code>input.length</code> elements are of relevance.
     *         <p>
     *         The returned array contains suffix indexes starting from 0 (so
     *         <code>start</code> needs to be added manually to access a given
     *         suffix in <code>input</code>).
     */
